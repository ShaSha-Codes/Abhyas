...
function StudentDashboard(props) {
    ...
    async function onSubmit(){
        ...
        if(data==""){
            ...
            let data1=await axios.patch("http://localhost:3000/student/add/teacher",{
                teacher:data.email,
                class:joinCode,
                email:ReactSession.get("data").email,
      ...
      <>
        <Navbar />
            ...
            <Stack alignItems={"center"}>
                <Button sx={{marginBottom:"30px",textAlign:"center",width:"20%"}} variant="contained" onClick={()=>setJoinVisibility(prev=>!prev)}>
                    Join a Class
                </Button>
                {
                    joinVisibility &&
                     ...
                     <Paper sx={{borderRadius:"10px"}} elevation={6}>
                         <Box p={2}>
                             <Stack alignItems={"center"} spacing={2}>
                                 <Typography sx={{marginBottom:"10px"}} textAlign={"center"} variant="h6" component="h1">
                                     Join Class
                                 </Typography>
                                 <TextField id="outlined-basic" label="Class Code" value={joinCode} onChange={handleChange} variant="outlined" />
                                 <Button sx={{textAlign:"center",width:"50%"}} variant="contained" onClick={onSubmit}>
                                     Join
                                 </Button>
                             </Stack>
                         </Box>
