...
function Questions() {
    ...
    React.useEffect(async ()=>{
        ...
        data=data.data[1]
        setInfo({title:data.title,description:data.description,code:data.code})
        let ques=data.qa
    ...
    async function handleSubmit(){
        ...
        if(status=="Passed"){
            let result=await axios.patch('http://localhost:3000/student/submit/quiz',{
                courseCode,
    ...
    function questionMaker(){
        let res=[];
        for(let i=0;i<questionContent.length;i++){
            res.push(<AskQuestionCard question={questionContent[i]} num={i} key={i} setQuestionContent={setQuestionContent}/>)
        }
        return res
