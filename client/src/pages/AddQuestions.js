...
export default function AddQuestions(){
    ...
    for(let x = 0 ; x < numberOfQuestions ; x++){
        rows.push(<div><h4 className="font-style" style={{marginLeft:"5%"}}>Question {x+1}</h4><QuestionInput key={x} id={x}/></div>)
    }
    ...
    return(
      ...
      />
        ...
        <Button sx={{marginBottom:"30px"}}  color="secondary" variant="contained">Submit</Button>
        </Box>
        </div>
    )
}